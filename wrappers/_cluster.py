...
class Cluster:
    ...
    def cluster_latent_space(self, color_by='leiden', umap_nneighbors = 10, cluster_aim=5):
        ...
        with warnings.catch_warnings():
            ...
            if 'batch' in adata.obs:
                ...
                sc.pl.umap(adata, color='batch', ax=axes[1], show=False, title='Clustering of Latent: Batch')
                plt.tight_layout()
                plt.show()
    ...
    def cluster_counts(self, color_by='leiden', cluster_omega = False, umap_nneighbors=10, cluster_aim=5):
        ...
        with warnings.catch_warnings():
            ...
            if 'batch' in adata_hat.obs:
                ...
                if (cluster_omega):
                    sc.pl.umap(adata_hat, color=color_by, ax=axes[0], show=False, title=r'Clustering of $\hat{\omega}$: ' + color_by)
                else:
                    sc.pl.umap(adata_hat, color=color_by, ax=axes[0], show=False, title=r'Clustering of Counts: ' + color_by)
                ...
                if (cluster_omega):
                    sc.pl.umap(adata_hat, color='batch', ax=axes[1], show=False, title=r'Clustering of $\hat{\omega}$: Batch')
                else:
                    sc.pl.umap(adata_hat, color='batch', ax=axes[1], show=False, title=r'Clustering of Counts: Batch')
                plt.tight_layout()
                plt.show()
            ...
            if 'subtype' in adata_hat.obs:
                ...
                if (cluster_omega):
                    sc.pl.umap(adata_hat, color='subtype', ax=axes[1], show=False, title=r'Clustering of $\hat{\omega}$: Subtype')
                else:
                    sc.pl.umap(adata_hat, color='subtype', ax=axes[1], show=False, title=r'Clustering of Counts: Subtype')
                plt.tight_layout()
                plt.show()
    ...
    def cluster_true_counts(self, color_by='leiden', umap_nneighbors=10, cluster_aim = 5):
        ...
        if 'batch' in adata[0].obs:
            fig, axes = plt.subplots(1, 2, figsize=(12, 6))
            sc.pl.umap(adata_true, color=color_by, ax=axes[0], show=False, title=r'Clustering of True Counts: ' + color_by)
            
            # In case of missing data
            adata_true.obs['batch'] = adata[0].obs['batch'].fillna(-1).astype('category')

            sc.pl.umap(adata_true, color='batch', ax=axes[1], show=False, title=r'Clustering of True Counts: Batch')
            plt.tight_layout()
            plt.show()
        ...
        if 'subtype' in adata[0].obs:
            ...
            sc.pl.umap(adata_true, color='subtype', ax=axes[1], show=False, title=r'Clustering of True Counts: Subtype')
